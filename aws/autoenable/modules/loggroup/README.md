# SumoLogic-CloudWatchLogGroup-Auto-Enable

This module is used to create AWS resources to automatically subscribe CloudWatch log groups to a destination for log forwarding. Features include:
- Create AWS Lambda function to automatically subscribe new CloudWatch log groups to a Lambda function or Kinesis Data Firehose stream
- Create AWS CloudWatch Event Rules to trigger Lambda function when new CloudWatch log groups are created
- Create AWS IAM roles with appropriate permissions for Lambda function to manage CloudWatch log group subscription filters
- Support for filtering log groups using regular expressions and tag-based matching
- Configurable options to subscribe new log groups only, existing log groups only, or both
- Support for Lambda and Kinesis Data Firehose as subscription filter destinations
- Option to provide an existing IAM role ARN when the destination is a Kinesis Data Firehose stream
