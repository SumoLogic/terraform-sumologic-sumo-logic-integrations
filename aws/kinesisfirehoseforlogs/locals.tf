locals {
  ...
  bucket_name = local.create_bucket && var.bucket_details.bucket_name == "sumologic-kinesis-firehose-logs-random-id" ? "sumologic-kinesis-firehose-logs-${random_string.aws_random.id}" : var.bucket_details.bucket_name

  # Get the default collector name if no collector name is provided.
  collector_name = var.collector_details.collector_name == "SumoLogic Kinesis Firehose for Logs Collector <Random ID>" ? "SumoLogic Kinesis Firehose for Logs Collector ${random_string.aws_random.id}" : var.collector_details.collector_name
